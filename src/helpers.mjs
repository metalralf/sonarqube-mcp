...
import { z } from 'zod';
import { resolveProjectKey, sonarGet } from './api.mjs';
...
export { resolveProjectKey, sonarGet };
...
export const LANG_CONFIGS = {
  python: {
    ...
    exclusions:
      'venv/**,.venv/**,__pycache__/**,*.pyc,*.pyo,.mypy_cache/,.pytest_cache/',
  ...
  typescript: {
    ...
    coverage: 'coverage/lcov.info',
    exclusions:
      'node_modules/**,bower_components/**,dist/**,build/**,**/*.d.ts',
    coverageProperty: 'sonar.javascript.lcov.reportPaths',
  },
  java: {
    sources: 'src/main',
...
export const detectLanguage = (dir) => {
  if (existsSync(join(dir, 'package.json'))) {
    try {
      ...
      if (deps?.typescript || deps?.tslib || deps?.['@types/node'])
        return 'typescript';
      /* c8 ignore next */
    } catch {}
    return 'javascript';
...
export const buildSonarProps = (projectKey, hostUrl, sources, lang, dir) => {
  ...
  if (cfg) {
    ...
    if (cfg.coverageProperty)
      props += `${cfg.coverageProperty}=${cfg.coverage}\n`;
...
const resolveDocker = () => {
  ...
  try {
    dockerPath = execSync('command -v docker', {
      encoding: 'utf8',
      timeout: 3000,
    }).trim();
  } catch {
    /* c8 ignore next */ dockerPath = '/usr/bin/docker';
  }
  return dockerPath;
};
...
export const hasDocker = () => {
  ...
  try {
    execSync(`${resolveDocker()} info`, { stdio: 'ignore', timeout: 5000 });
    return true;
  } catch {
...
export const getDockerImage = () =>
  process.env.SONARQUBE_DOCKER_IMAGE || 'sonarsource/sonar-scanner-cli:11.1';

/**
 * Get Docker run flags.
 * @returns {string} — override with SONARQUBE_DOCKER_FLAGS (e.g. '--network=bridge' or '')
 */
export const getDockerFlags = () =>
  process.env.SONARQUBE_DOCKER_FLAGS ?? '--network=host';

/**
 * Get scanner timeout in ms.
 * @returns {number} — override with SONARQUBE_SCANNER_TIMEOUT (default 300000 = 5 min)
 */
export const getScannerTimeout = () =>
  Number.parseInt(process.env.SONARQUBE_SCANNER_TIMEOUT || '300000', 10);

/**
 * Get Docker mount path inside container.
 * @returns {string} — override with SONARQUBE_DOCKER_MOUNT_PATH (default /usr/src)
 */
export const getDockerMountPath = () =>
  process.env.SONARQUBE_DOCKER_MOUNT_PATH || '/usr/src';
...
export const getSourceContext = () =>
  Number.parseInt(process.env.SONARQUBE_SOURCE_CONTEXT || '2', 10);
...
export const autoBuild = (dir, langCfg) => {
  ...
  if (hasGradle) {
    execSync(
      `${existsSync(join(dir, 'gradlew')) ? './gradlew' : 'gradle'} build -x test`,
  ...
  if (hasMaven) {
    execSync(
      `${existsSync(join(dir, 'mvnw')) ? './mvnw' : 'mvn'} compile -DskipTests`,
      { cwd: dir, encoding: 'utf8', timeout: 300000 },
    );
    return { performed: true };
  }
...
export const detectMultiModule = (dir) => {
  const settings = ['settings.gradle', 'settings.gradle.kts'].find((f) =>
    existsSync(join(dir, f)),
  );
  if (settings) {
    const text = readFileSync(join(dir, settings), 'utf8');
    if (text.includes('include '))
      return { hasSubmodules: true, type: 'Gradle' };
  }
  const pom = join(dir, 'pom.xml');
  if (existsSync(pom)) {
    const text = readFileSync(pom, 'utf8');
    if (text.includes('<module>'))
      return { hasSubmodules: true, type: 'Maven' };
  }
...
const resolveGit = () => {
  ...
  try {
    gitPath = execSync('command -v git', {
      encoding: 'utf8',
      timeout: 3000,
    }).trim();
  } catch {
...
export const detectGitBranch = (dir) => {
  try {
    const branch = execSync(`${resolveGit()} rev-parse --abbrev-ref HEAD`, {
      cwd: dir,
      encoding: 'utf8',
      timeout: 5000,
    }).trim();
...
const runLocalScanner = (dir, baseArgs) => {
  const scannerBin = existsSync(
    join(dir, 'node_modules', '.bin', 'sonar-scanner'),
  )
    ? join(dir, 'node_modules', '.bin', 'sonar-scanner')
    : 'sonar-scanner';
  return execSync(`${scannerBin} ${baseArgs.join(' ')}`, {
    cwd: dir,
    encoding: 'utf8',
...
export const buildScannerHints = (output, lang) => {
  const hints = [];
  if (output.includes('No coverage'))
    hints.push(
      'Coverage report was not found. Run tests with coverage enabled before analysis (e.g. ./gradlew test jacocoTestReport).',
    );
  if (output.includes('Missing blame information') && lang === 'java')
    hints.push(
      'SCM blame info is missing. Run analysis from the project root directory with git history.',
    );
  return hints;
};
...
export const pollCeTask = async (
  ...
  while (Date.now() < deadline) {
    ...
    if (status === 'FAILED' || status === 'CANCELED')
      throw new Error(
        `CE task ${status.toLowerCase()}: ${data.task?.errorMessage || 'Unknown error'}`,
      );
...
export const mapScannerError = (msg) => {
  if (msg.includes("can't be indexed twice"))
    return 'Your sonar.sources and sonar.tests paths overlap. Set sonar.sources=src/main (or the correct source directory) and sonar.tests=src/test.';
  if (
    msg.includes('No files nor directories matching') ||
    msg.includes('sonar.java.binaries')
  )
    return 'No compiled class files found. Build the project first (e.g. ./gradlew build) or set sonar.java.binaries to the correct path.';
  return undefined;
};
...
export const detectSourceLanguages = (dir, sources = 'src') => {
  ...
  const walk = (d, depth) => {
    ...
    for (const e of entries) {
      if (e.isDirectory()) {
        if (!SKIP_DIRS.has(e.name)) walk(join(d, e.name), depth + 1);
      } else {
        if (e.name === 'Dockerfile' || e.name.startsWith('Dockerfile.'))
          langs.add('docker');
...
export const detectTestsDir = (dir) => {
  ...
  try {
    const entry = readdirSync(dir, { withFileTypes: true }).find(
      (e) => e.isDirectory() && /\.Tests$/i.test(e.name),
    );
...
const COVERAGE_FILES = [
  ...
  },
  {
    ...
    property: 'sonar.coverage.cobertura.reportPaths',
  },
];
 ...
 * @returns {boolean}
 */
export const hasExistingSonarConfig = (dir) =>
  SONAR_CONFIG_FILES.some((f) => existsSync(join(dir, f)));
...
export const detectSourcesDir = (dir) => {
  ...
  try {
    if (readdirSync(src, { withFileTypes: true }).length === 0) return '.';
  } catch {
...
export const projectKey =
  /** @type {import('zod').ZodOptional<import('zod').ZodString>} */ (
    z
      .string()
      .optional()
      .describe('Project key (defaults to SONARQUBE_PROJECT)')
  );
export const componentKey = /** @type {import('zod').ZodString} */ (
  z.string().describe('Full component key (e.g. my-project:src/file.ts)')
);
export const maxResults =
  /** @type {import('zod').ZodOptional<import('zod').ZodNumber>} */ (
    z.number().optional().describe('Max results (default 50, max 500)')
  );
export const branch =
  /** @type {import('zod').ZodOptional<import('zod').ZodString>} */ (
    z
      ...
      .optional()
      .describe(
        'Long-lived branch name (e.g. main, develop). Use sonar_list_branches to discover valid names.',
      )
  );
export const pullRequest =
  /** @type {import('zod').ZodOptional<import('zod').ZodString>} */ (
    z
      ...
      .optional()
      .describe(
        'Pull request key/ID. Use sonar_list_pull_requests to discover valid keys.',
      )
  );
...
export const measureSearch =
  (metricKey, valueKey, defaultThresh, descend) =>
  async ({ projectKey, branch, pullRequest, threshold }) => {
    ...
    const extract = (/** @type {any} */ m) => ({
      path: m.component.split(':').pop(),
      [valueKey]: Number.parseFloat(m.value),
    });
    const items = (data.measures || []).filter(
      (/** @type {any} */ m) =>
        m.value !== undefined && m.component !== key && m.component,
    );
    const sorted = items
      .map(extract)
      .filter((/** @type {any} */ f) =>
        descend ? f[valueKey] > t : f[valueKey] < t,
      )
      .sort((/** @type {any} */ a, /** @type {any} */ b) =>
        descend ? b[valueKey] - a[valueKey] : a[valueKey] - b[valueKey],
      );
...
export const filterTools = (all) => {
  ...
  if (envToolsets) {
    ...
    if (enabled.size)
      return all.filter(
        (t) =>
          enabled.has(t.name) && !(readOnly && READ_ONLY_TOOLS.has(t.name)),
      );
  }
